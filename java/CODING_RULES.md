...
4.	コメント	14
4.1.	クラスのJavadocコメント	14
4.2.	フィールドのJavaDocコメント	14
4.3.	メソッドのJavaDocコメント	15
4.4.	行コメント	16

5.	禁止事項	17
5.1.	計算式の途中でインクリメント・デクリメントをしない	17
5.2.	フィールドを一時変数として使用しない	18
5.3.	スーパークラスと同名のフィールドをサブクラスで定義しない	19
5.4.	戻り値がコレクションや配列の場合は null を返さない	20
5.5.	コンストラクタ内で自分自身のインスタンスメソッドを呼び出さない	21
5.6.	インスタンス変数をレシーバとしたstatic メソッド呼び出しをしない	23
5.7.	インスタンス変数をレシーバとしたstatic メソッド参照をしない	23
5.8.	独自例外クラスを実装しない	24
5.9.	java.lang.Exceptionクラスのインスタンスを生成してスローをしない	24
5.10.	try-catch 文を条件分岐のために使用しない	25
5.11.	秘匿情報はログ出力やシリアライズされないように注意すること	25
5.12.	Java標準ライブラリにあるレガシーなAPIは使用しない	26
5.13.	リフレクションを直接使用しない	26
5.14.	クラスを不整合な状態にしない	27
5.15.	独自非同期クラスを実装しない	29
5.16.	名前と値が同じ定数は作成しない	29

6.	注意事項	30
6.1.	メソッドには名前から想像できない処理を実装しない	30
6.2.	単一のメソッドに複数の要素を詰め込まない	31
6.3.	クラス外に公開されるメソッドの引数や戻り値・フィールドの型は実装クラスではなくインターフェースで宣言すること	31
6.4.	メソッドのオーバーロードはオプションの省略用途のみに使用すること	33
6.5.	未使用コードは残したままにしない	34
6.6.	クラスは大きくなりすぎないようにする	34
6.7.	メソッドは大きくなりすぎないようにする	34
6.8.	インナークラスやstaticにネストしたクラス、匿名クラスは作りすぎない	34
6.9.	可能な限りキャストは使用しない	35
6.10.	アンボクシングの注意事項	36
6.11.	計算の誤差が許されない場合はBigDecimalを使用すること	37
6.12.	ソートと集計はJava側ではなく、SQLで行うようにする	38
6.13.	ループ処理の中では極力データベースアクセスしないようにする	38
6.14.	外部からの入力値は共通部品を用いてチェックする	39
6.15.	ファイルの入出力は共通部品のクラスを使用すること	39
6.16.	リソースをクローズする必要がある場合はtry-with-resources構文を使用すること	40
6.17.	例外処理はプロジェクトの方式設計に従って統一的にコーディングすること	40
6.18.	ループのネストはなるべく少なくすること	40

7.	推奨事項	41
7.1.	クラスやメソッドなどには適切なアクセス修飾子を付与する	41
7.2.	インスタンス変数は原則 private に設定する	41
7.3.	ローカル変数はできるだけ狭いスコープで使用する	42
7.4.	なるべく再代入は避けて、finalを活用する	44
7.5.	引数の状態をなるべく変えない	45
7.6.	戻り値として null を使いたい場合、Optional の使用を検討する	46
7.7.	コレクション等の型パラメーターを取るクラスを使用する場合はバインドを活用する	48
7.8.	コレクション処理時のStream API活用を検討する	48
7.9.	チェック例外のスローを伴う処理はStream APIではなく拡張for文を活用する	50
7.10.	コレクションの順次処理にはレガシーfor文はなるべく使用しない	51
7.11.	配列全体をコピーする場合はcloneメソッドを使用する	51
7.12.	コレクションを配列に変換する場合はtoArrayメソッドを使用する	52
7.13.	配列をコレクションに変換する場合はコレクションのofメソッドを使用する	53
7.14.	メソッドのオーバーライドや、抽象メソッドを実装する場合はメソッドに@Overrideを付与する	54
7.15.	複数行の文字列定義はテキストブロックが使用できるか検討する	56
7.16.	変数に代入する値を分岐で切り替えている場合、switch式を検討する	58
